...
public record Stream<C0>(Query Query, Match Match0) : IEnumerable<(Entity entity, C0 comp0)>, IBatchBegin 
    where C0 : notnull
{
    ...
    /// <inheritdoc />
    public IEnumerator<(Entity entity, C0 comp0)> GetEnumerator()
    {
        foreach (var table in Filtered)
        {
            using var join = table.CrossJoin<C0>(_streamTypes.AsSpan());
